...
router.post('/signup', function (req, res) {
  ...
  try {
    ...
    if (user) {
      return res.status(400).json({
        message:
          'Ошибка. Пользованель с таким e-mail уже зарегистрирован.',
      })
    }
...
router.post('/recovery-confirm', function (req, res) {
  ...
  try {
    ...
    if (!email) {
      return res.status(400).json({
        message: 'Ошибка. Такого кода не существует.',
      })
    }
    ...
    if (!user) {
      return res.status(400).json({
        message:
          'Ошибка. Пользователя с таким e-mail не существует.',
      })
    }
...
router.post('/signup-confirm', function (req, res) {
  ...
  try {
    ...
    const user = User.getByEmail(session.user.email) //! ? оставлять или нет?
    user.isConfirm = true //! ?
...
router.post('/login', function (req, res) {
  ...
  try {
    ...
    if (!user) {
      return res.status(400).json({
        message:
          'Ошибка. Пользователь с таким e-mail не зарегистрирован.',
      })
    }
