...
class UserItem extends List {
  constructor() {
    ...
    this.id = new URLSearchParams(location.search).get('id') // то же самое, что и выше
  ...
  loadData = async () => {
    ...
    try {
      const res = await fetch(
        ...
        {
          method: 'GET',
          // body и  headers тут нет, т к никаких данных мы не передаем тут, а получаем
        },
      )
  ...
  convertData = (data) => {
    return {
      ...
      user: {
        ...
        // isConfirm: true ? 'Да' : 'Нет',  // тоже работает
        confirm: data.user.isConfirm ? 'Да' : 'Нет',
      },
    }
  }
    ...
    switch (this.status) {
      case this.STATE.LOADING:
        this.element.innerHTML = `
					...
					<div class="data">
						<span class="data__title ">Почта подтверждена?</span>
						<span class="data__value skeleton"></span>
					</div>
				`
        break
